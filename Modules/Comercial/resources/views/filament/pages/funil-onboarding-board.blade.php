            ...
            >
                ...
                <div class="flex flex-col gap-1.5 min-h-[150px]">
                    @foreach(collect($onboardings)->where('status', $stage) as $op)
                        ...
                        @endphp
                        <div class="bg-white dark:bg-gray-800 p-2 rounded shadow-sm border-l-4 border-indigo-500 cursor-grab hover:shadow-md transition-all flex flex-col gap-1"
                             draggable="true"
                             @dragstart="draggedId = {{ $op['id'] }}; $event.dataTransfer.effectAllowed = 'move';"
                        >
                            <div class="flex items-center justify-between">
                                @if(!empty($op['oportunidade_id']))
                                    <a href="{{ \Modules\Comercial\Filament\Resources\OportunidadeResource::getUrl('view', ['record' => $op['oportunidade_id']]) }}" class="font-bold text-left text-gray-800 dark:text-gray-100 hover:text-indigo-600 dark:hover:text-indigo-400 truncate w-full text-xs leading-tight">
                                        {{ $op['titulo'] }}
                                    </a>
                                @else
                                    <button type="button" wire:click="mountAction('editOnboarding', { record: {{ $op['id'] }} })" class="font-bold text-left text-gray-800 dark:text-gray-100 hover:text-indigo-600 dark:hover:text-indigo-400 truncate w-full text-xs leading-tight">
                                        {{ $op['titulo'] }}
                                    </button>
                                @endif
                                <button type="button" wire:click="mountAction('editOnboarding', { record: {{ $op['id'] }} })" class="text-gray-400 hover:text-indigo-600 transition-colors flex-shrink-0 ml-1">
                            ...
                            <div class="flex justify-between items-end mt-1.5 pt-1.5 border-t border-gray-50 dark:border-gray-700/50">
                                <div class="font-bold text-indigo-600 dark:text-indigo-400 text-[11px]">
