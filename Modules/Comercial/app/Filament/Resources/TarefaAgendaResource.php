                ...
                \Filament\Schemas\Components\Section::make('Detalhes da Tarefa')
                    ->schema([
                        ...
                        Forms\Components\Select::make('oportunidade_id')
                            ...
                            ->searchable()
                            ->preload(),
                        Forms\Components\Select::make('etapa_funil')
                            ...
                            ->afterStateHydrated(function ($component, $state, ?\Illuminate\Database\Eloquent\Model $record) {
                                if ($record && $record->oportunidade) {
                                    $component->state($record->oportunidade->status);
                                }
                            })
                            ->live()
                            ->helperText('Ao selecionar uma etapa e salvar, esta prospecção será atualizada com o progresso no funil.')
                            ->columnSpanFull(),
                        Forms\Components\Radio::make('acao_oportunidade')
                            ->label('Ação da Oportunidade')
                            ->options([
                                'atualizar' => 'Atualizar Oportunidade Atual',
                                'criar' => 'Criar Nova Oportunidade',
                            ])
                            ->default('atualizar')
                            ->visible(fn ($get) => $get('oportunidade_id') && $get('etapa_funil'))
                            ->inline()
                            ->columnSpanFull(),
